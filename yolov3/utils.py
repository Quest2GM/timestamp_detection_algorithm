...
def draw_bbox(image, bboxes, CLASSES=YOLO_COCO_CLASSES, show_label=True, show_confidence=True,
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if NUM_CLASS[class_ind] == "time_stamp":
            print("Start X:", x1, "Start Y:", y1, "End X:", x2, "End Y:", y2)
            stamp_locs.append([x1, y1, x2, y2])
...
def detect_image(YoloV3, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES,
                 score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    orig_image = cv2.imread(image_path)
    original_image = cv2.cvtColor(orig_image, cv2.COLOR_BGR2RGB)
    ...
    image, stamp_locs = draw_bbox(original_image, bboxes, CLASSES=CLASSES, rectangle_colors=rectangle_colors)
    ...
    return orig_image, image, stamp_locs
